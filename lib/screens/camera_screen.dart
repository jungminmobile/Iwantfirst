...
class _CameraScreenState extends State<CameraScreen> {
  ...
  void initState() {
    ...
    // 🟢 [추가] 컨트롤러 초기화 (오늘 날짜가 맨 끝이므로 index 6)
  ...
  Future<void> _loadTempData() async {
    ...
    setState(() {
      ...
      _breakfastImages = (prefs.getStringList(_key('breakfast_images')) ?? []).map((path) => XFile(path)).toList();
      _lunchImages = (prefs.getStringList(_key('lunch_images')) ?? []).map((path) => XFile(path)).toList();
      _dinnerImages = (prefs.getStringList(_key('dinner_images')) ?? []).map((path) => XFile(path)).toList();
      _snackImages = (prefs.getStringList(_key('snack_images')) ?? []).map((path) => XFile(path)).toList();
    });
  }
  ...
  // 🟢 [신규] 날짜 변경 시 호출되는 함수 (스와이프 or 탭)
  Future<void> _onDateChanged(int index) async {
    // 1. 이동하기 전 데이터를 먼저 저장 (안전장치)
    await _saveTempData();

    setState(() {
      // 2. 날짜 변경 및 데이터 초기화 (잔상 제거)
  ...
  // ... (기존 _retrieveLostData, _pickImage 등 함수들은 그대로 유지) ...
  Future<void> _retrieveLostData() async {
    final LostDataResponse response = await _picker.retrieveLostData();
    if (response.isEmpty) return;
    final XFile? file = response.file;
    if (file != null) {
      setState(() { _breakfastImages.add(file); });
      _saveTempData();
    }
  }
  ...
  Future<void> _pickImage(String mealType, ImageSource source) async {
    try {
      final XFile? pickedFile = await _picker.pickImage(source: source, imageQuality: 50, maxWidth: 1024);
      if (pickedFile != null) {
        setState(() {
          switch (mealType) {
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: TextField(
            ...
            decoration: const InputDecoration(hintText: '예: 현미밥 1공기', border: OutlineInputBorder()),
            maxLines: 3,
          ),
          actions: [
      ...
      child: Stack(
        children: [
          Scaffold(
            ...
            body: NestedScrollView( // 🟢 CustomScrollView 대신 NestedScrollView 사용 (SliverFillRemaining 버그 방지)
              ...
              // 🟢 [핵심] 몸통을 PageView로 변경하여 스와이프 가능하게 함
              body: Container(
                color: Colors.white,
                child: PageView.builder(
                  ...
                  itemCount: 7,
                  onPageChanged: (index) {
                    // 🟢 페이지가 넘어갈 때 데이터 로드 함수 호출
                    _onDateChanged(index);
                  },
                  itemBuilder: (context, index) {
                    // 각 페이지의 내용 (스크롤 가능한 식단 리스트)
  ...
  // ... _buildMealSection, _buildSummaryCard, _buildMacroInfo, _buildInputForm 코드는 기존과 동일 ...
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('${data['totalCalories']} kcal', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.black87)),
            ],
          ),
        ...
        (images.isEmpty && textItems.isEmpty)
          ...
          children: [
              ...
              SizedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return Stack(
                      children: [
                        Container(width: 100, height: 100, margin: const EdgeInsets.only(right: 10, top: 5), decoration: BoxDecoration(color: Colors.grey[200], borderRadius: BorderRadius.circular(8), image: DecorationImage(image: FileImage(File(images[index].path)), fit: BoxFit.cover))),
                        Positioned(right: 5, top: 0, child: GestureDetector(onTap: () => _removeImage(title, images[index]), child: Container(padding: const EdgeInsets.all(4), decoration: const BoxDecoration(color: Colors.black54, shape: BoxShape.circle), child: const Icon(Icons.close, size: 14, color: Colors.white)))),
                      ],
                    );
                  },
                ),
            ...
            if (textItems.isNotEmpty)
              Wrap(spacing: 8.0, runSpacing: 4.0, children: textItems.map((text) { return Chip(label: Text(text), backgroundColor: Colors.lightBlue[50], side: BorderSide.none, onDeleted: () => _removeText(title, text)); }).toList()),
          ],
        ),
        const SizedBox(height: 15),
        if (images.isNotEmpty || textItems.isNotEmpty)
          SizedBox(width: double.infinity, child: ElevatedButton.icon(onPressed: () => _onAnalyzePressed(title), icon: const Icon(Icons.analytics_outlined, size: 18), label: const Text('분석 시작'), style: ElevatedButton.styleFrom(backgroundColor: Colors.blue[50], foregroundColor: Colors.blue[700], elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), padding: const EdgeInsets.symmetric(vertical: 12)))),
      ],
    );
  }
}

// ... ExpandableFab, _InvertedCorner, _InvertedCornerPainter 클래스는 기존과 동일 (아까 드린 코드 그대로 사용) ...
class ExpandableFab extends StatefulWidget {
  final VoidCallback onCameraTap;
  final VoidCallback onGalleryTap;
  final VoidCallback onTextTap;

  const ExpandableFab({super.key, required this.onCameraTap, required this.onGalleryTap, required this.onTextTap});
...
class _ExpandableFabState extends State<ExpandableFab> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      mainAxisAlignment: MainAxisAlignment.end,
      children: [
        AnimatedSize(duration: const Duration(milliseconds: 200), curve: Curves.easeOut, child: SizedBox(width: _isOpen ? null : 0, child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(mainAxisSize: MainAxisSize.min, children: [_buildActionBtn(Icons.camera_alt, '카메라', widget.onCameraTap), const SizedBox(width: 8), _buildActionBtn(Icons.photo_library, '갤러리', widget.onGalleryTap), const SizedBox(width: 8), _buildActionBtn(Icons.edit, '텍스트', widget.onTextTap), const SizedBox(width: 12)])))),
        GestureDetector(onTap: _toggle, child: AnimatedRotation(turns: _isOpen ? 0.125 : 0, duration: const Duration(milliseconds: 200), child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: _isOpen ? Colors.grey[200] : Colors.blue[50], shape: BoxShape.circle), child: Icon(Icons.add, size: 18, color: _isOpen ? Colors.grey : Colors.blue)))),
      ],
    );
  }

  Widget _buildActionBtn(IconData icon, String tooltip, VoidCallback onTap) {
    return GestureDetector(onTap: () { _toggle(); onTap(); }, child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.blue, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 4, offset: const Offset(0, 2))]), child: Icon(icon, size: 20, color: Colors.white)));
  }
}
...
class _InvertedCorner extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) { return SizedBox(width: 12, height: 12,
      child: CustomPaint(painter: _InvertedCornerPainter(color: color, isLeft: isLeft))); }
}
...
class _InvertedCornerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path = Path();
    if (isLeft) { path.moveTo(size.width, 0);
