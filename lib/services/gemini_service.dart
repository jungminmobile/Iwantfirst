  ...
  Future<List<Map<String, String>>?> identifyFoodList(List<XFile> images, List<String> texts) async {
    try {
      ...
      final promptText = """
       ...
       1-1. 음식의 메뉴를 추정할 때 최대한 상세하게 추측해. 예를 들어 탄산음료라면 콜라, 사이다, 과일 쥬스.
       치킨이라면 양념치킨, 간장치킨, 후라이드 치킨.
       1-2. 한식 메뉴를 최대한 정확하게 추측해. 검색 기능을 사용해도 좋아.
  ...
  Future<String?> generateAdvice(String nutritionAnalysisJson, Map<String, dynamic> userData) async {
    try {
      ...
      final promptText = """
        ...
        다시 한 번 말하는데, 가장 중요한건 자연어만 출력하는거야. 출력 값이 {advice: ,....} 이런식으로 나오지 않게, 자연어만 출력해.
